...
def create_order(): # Dit stond niet in de oefening, maar deze functie moet buiten de class worden geplaatst. Dus na een gesprek met een Winc mentor op dinsdag 17-10-2023 tussen 11:00 en 11:45\
                    # uur ben ik hierachter gekomen.

    customer_name = input("Please enter your name: ")   # Met de 'input()' code zorg je ervoor dat de gebruiker iets in kan vullen. LET OP!!! De input code werkt ook zonder dat je de variabel\
    # print(customer_name)                              # definieert.
    
    pizza_type = input("Please choose your pizza type: ")
    # print(pizza_type)

    toppings = input("Please choose your toppings and please separate each topping with a comma: ").split(",")  # Door de 'split()' code hier te gebruiken met de komma als separator, zorg je\
    # print(toppings)                                                                                           # ervoor dat de items in deze input worden gescheiden door de komma.
...
Define a function called 'save_orders_to_file' in the 'order_management' module, which takes a list of 'PizzaOrder' objects (order_list, for example) and saves them to a file named "orders.txt"\
in the current directory. Each PizzaOrder object should be written to a new line in the file.

Create a 'orders.txt' file in your project folder. Use the with statement to open the file: 'with open('./orders.txt', 'w') as file:' With this we are opening the file by giving the path and\
writing mode as arguments.
